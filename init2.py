...
current_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'openatx')
...
class Installer(adbutils.Adb):
    ...
    def install_minicap(self):
        ...
        path = os.path.join(current_path, 'stf-binaries-'+stf_binaries_version, 'node_modules', 'minicap-prebuilt', 'prebuilt',
                            self.abi, 'lib','android-' + sdk, 'minicap.so')
        exedir = self.get_executable_dir()
        minicapdst = "%s/%s" % (exedir, 'minicap.so')
        self.push(path, minicapdst)
        log.info("install minicap")
        path = os.path.join(current_path, 'stf-binaries-'+stf_binaries_version, 'node_modules', 'minicap-prebuilt', 'prebuilt',
                            self.abi, 'bin', 'minicap')
    ...
    def install_uiautomator_apk(self, apk_version, reinstall=False):
        ...
        path = os.path.join(current_path, 'uiautomator', __apk_version__, 'app-uiautomator.apk')
        pathtest = os.path.join(current_path, 'uiautomator', __apk_version__, 'app-uiautomator-test.apk')
        self.install(path)
    ...
    def install_atx_agent(self, agent_version, reinstall=False):
        ...
        if not name:
            raise Exception(
                ...
                % abis)
        path = os.path.join(current_path, 'atx-agent', __atx_agent_version__, name.format(v=__atx_agent_version__))
